...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    ps = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def about(subject):
    ...
    def pick(paragraphs):
        for word in subject:
            if word in remove_punctuation(paragraphs.lower()).split():
                return True
        return False
    return pick
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not typed_words and not source_words:
        ...
        correct_words = 0
        for w1, w2 in zip(typed_words, source_words):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        closest_word, closest_diff = min([(word, diff) for (word, diff) in zip(word_list, word_diff)], 
                                         key=lambda pair: pair[1])
        if closest_diff <= limit:
            return closest_word
        else:
            return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    5
    """
    # BEGIN PROBLEM 6 # This implement exceeds the limit of recursion in the autograder
    # def autocorrect_helper(typed, source, limit, num_chars):
    #     if num_chars > limit or not typed or not source:
    #         return num_chars
    #     elif typed[0] != source[0]:
    #         return autocorrect_helper(typed[1:], source[1:], limit, num_chars+1)
    #     else:
    #         return autocorrect_helper(typed[1:], source[1:], limit, num_chars)
    # return autocorrect_helper(typed, source, limit, abs(len(typed)-len(source)))
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times_per_player = [[timestamp[i+1] - timestamp[i] for i in range(len(timestamp)-1)]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    times_per_word = [[time(match, player_num, word_index) for player_num in player_indices]
                      for word_index in word_indices]
    fastest_indices = [min(range(len(t)), key=t.__getitem__) for t in times_per_word]
    fastest = [[] for _ in player_indices]
    for word_index, player in enumerate(fastest_indices):
