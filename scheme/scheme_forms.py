...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        name, parameters, body = signature.first, signature.rest, expressions.rest
        env.define(name, do_lambda_form(Pair(parameters, body), env))
        return name
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    if body is nil:
        raise SchemeError('body cannot be nil')
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names, vals = Pair(symbol, names), Pair(scheme_eval(expression, env), vals)
